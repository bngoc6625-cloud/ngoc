...
# --- HÀM PHÁT ÂM THANH (Dùng hiệu ứng âm thanh hệ thống) ---
def play_audio(sound_type):
    # Đường dẫn âm thanh mẫu (Bạn có thể thay link .mp3 khác nếu muốn)
    correct_url = "https://www.soundjay.com/misc/sounds/bell-ringing-05.mp3"
    wrong_url = "https://www.soundjay.com/misc/sounds/fail-trombone-01.mp3"
    url = correct_url if sound_type == "correct" else wrong_url
...
# --- DỮ LIỆU CÂU HỎI (Giữ nguyên cấu trúc) ---
questions_data = {
    "Dễ (10 câu)": [
...
if 'state' not in st.session_state:
    st.session_state.update({
        'state': "MENU", 'score': 0, 'current_idx': 0, 
        'wrong_list': [], 'total_attempted': 0
    })
...
if st.session_state.state == "MENU":
    ...
    st.write("Chọn mức độ (Đúng tổng 22 câu để đạt Đẳng Cấp!):")
    ...
    for i, opt in enumerate(q['options']):
        if cols[i%2].button(opt, key=f"q_{st.session_state.current_idx}_{i}", use_container_width=True):
            ans_selected = opt

    # Chạy đếm ngược nếu chưa chọn
    if ans_selected is None:
        for t in range(10, -1, -1):
            timer_place.metric("⏳ Thời gian", f"{t}s")
            time.sleep(1)
            if t == 0:
                st.error("Hết giờ!")
                play_audio("wrong")
        ...
        if ans_selected == q['a']:
            ...
            play_audio("wrong")
            st.session_state.wrong_list.append(q)
            time.sleep(3) # Cho thời gian đọc giải thích
    ...
    st.header(f"Bạn đúng: {s} câu")
    
    if s >= 22: st.balloons(); st.success("🌟 Bạn thật là đẳng cấp!")
    elif s < 15: st.warning("📚 Bạn cần ôn tập kĩ hơn.")
    
    if st.button("Làm lại"): st.session_state.state = "MENU"; st.rerun()
    if st.button("Ôn tập câu sai"): st.session_state.state = "REVIEW"; st.rerun()
